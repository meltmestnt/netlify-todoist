...
function RoutePreloader(props) {
  const { classes, location, closeMenu, menu } = props;
  console.log(props);
  const [showLoader, toggleLoader] = React.useState(true);
  const [oldLocation, changeLocation] = React.useState(location);
  const theme = useTheme();
  const firstDiv = useTransition(showLoader, null, {
    config: config.stiff,
    from: { width: "0%" },
    enter: { width: "100vw" },
    leave: { width: "0%", right: 0 }
  });
  const [logoProps, set, stop] = useSpring(() => ({
    config: config.stiff,
      ...
      {firstDiv.map(({ item, props, key }) =>
        item ? (
          ...
          >
            ...
            >
              <svg height="32" width="127" class="_1677nciMWXLkZVwCabecAd">
                <mask id="logo-wordmark_svg__a" fill="#fff">
            ...
            <Container maxWidth={"lg"} className={classes.container}>
              <Grid container className={classes.container}>
                <div style={{ width: "333px", position: "relative" }}>
                  <Menu menuOpen={menu} handleMenuClose={closeMenu}></Menu>
                </div>
                <Grid item xs={12} className={classes.mainContainer}>
                  <Switch location={oldLocation}>
                    <Route
                      exact
                      path="/"
                    ...
                    <Route
                      path="/сегодня"
                      render={() => (
                        <TodayContent classes={classes}></TodayContent>
                      )}
                    ></Route>
                    <Route
                      path="/завтра"
                    ...
                    <Route
                      ...
                      exact
                      render={() => (
                        <WeekContent classes={classes}></WeekContent>
                      )}
                    ></Route>
                    <Route
                      path="/неделя/:id"
